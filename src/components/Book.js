...
function BestBook(props) {
  const { bookTitle } = props;
  return (
    <div id="book_title_div" className="yellow_text">
      {bookTitle}
    </div>
  );
}
...
function BookTitle(props) {
  ...
  switch (bookType) {
    ...
    case 'best':
      return (
        <p>
          <a className="book_a_tag yellow_text" href={bookUrl}>
    ...
    return (
      <div>
        <div className="ui container left aligned">
          <BookTitle bookTitle={data.title} bookType={data.book_type} bookUrl={data.amazon_url} />
        </div>
      </div>
    );
  }
